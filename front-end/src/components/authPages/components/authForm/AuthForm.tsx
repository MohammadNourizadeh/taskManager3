...
export default function AuthForm({ signUp = false }: { signUp?: boolean }) {
  ...
  // func
  const handleFetch = async (isSignUp: boolean = false) => {
    let url: string;
  ...
  const handleInputErr = (
    inputName: keyof AuthenticationErrorType,
    errMessage: string
  ) => {
    setErrors((prev) => ({
      ...prev,
    ...
    >
      {signUp && (
        <div className={styles.inputsContainer}>
          ...
          {errors.emailErr !== "" && (
            <InputErrMessage
              errMessage={errors.emailErr}
              conditionState1={emailInput}
            />
          )}
        </div>
      )}
      <div className={styles.inputsContainer}>
        ...
        {errors.usernameErr !== "" && (
          <InputErrMessage
            errMessage={errors.usernameErr}
            conditionState1={usernameInput}
          />
        )}
      </div>
      <div className={styles.inputsContainer}>
        ...
        <div className={styles.passwordInput}>
          <input
            ...
            className={
              errors.passwordErr !== "" && passwordInput === ""
                ? styles.inputWithError
                : ""
            }
          />
        ...
        {errors.passwordErr !== "" && (
          <InputErrMessage
            errMessage={errors.passwordErr}
            conditionState1={passwordInput}
          />
        )}
      </div>
      {signUp && (
        <div className={styles.inputsContainer}>
          ...
          <div className={styles.passwordInput}>
            <input
              ...
              className={
                (errors.confirmPasswordErr === "Confirm your password" &&
                  confirmPasswordInput === "") ||
          ...
          {errors.confirmPasswordErr !== "" && (
            <InputErrMessage
              conditionState1={confirmPasswordInput}
              errMessage={errors.confirmPasswordErr}
              conditionState2={passwordInput}
            />
          )}
        </div>
      )}
